...
public class CidadeService {
  ...
  private static String ESTADO_NAO_INFORMADO =
      "Para cadastrar uma cidade é preciso informar um estado";
  private static String CIDADE_NAO_CADASTRADA = "Não existe uma cidade cadastrada com este ID: %d";
  private static String CIDADE_JA_CADASTRADA =
      "Já existe uma cidade cadastrada com estas informações:\n Cidade: %s  Estado: %s";
  ...
  public Cidade atualizar(Long id, Cidade cidade) {
    ...
    Cidade cidadeCadastrada =
        cidadeRepository
            .findById(id)
            .orElseThrow(
                () -> new EntidadeNaoCadastradaException(String.format(CIDADE_NAO_CADASTRADA, id)));
  ...
  private void deveCadastarCidadeOuLancarExcecao(Cidade cidade) {
    ...
    boolean existeCidade =
        cidadeRepository
            .findByNomeAndEstadoId(cidade.getNome(), cidade.getEstado().getId())
            .isPresent();
